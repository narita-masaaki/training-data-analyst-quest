...
def run():
    ...
    options.view_as(GoogleCloudOptions).job_name = '{0}{1}'.format('batch-user-traffic-pipeline-sql-', time.time_ns())  # ジョブ名
    ...
    agg_table_name = opts.agg_table_name  # 集計結果のテーブル名
    raw_table_name = opts.raw_table_name  # 生データのテーブル名

    # ここでは、入力ファイルのパスと出力先の BigQuery テーブルを指定しています。


    # 4.4. BigQueryのテーブルスキーマ
    raw_table_schema = {  # 生データのスキーマを定義
        "fields": [
            {"name": "ip", "type": "STRING"},
    ...
    # ここでは、生データと集計結果のそれぞれのテーブルのスキーマを定義しています。
    ...
    # このクエリは、ユーザーごとにページビュー数、合計バイト数、最大バイト数、最小バイト数を計算します。
    ...
    # 4.7. データの読み込み、変換
    logs = (p | 'ReadFromGCS' >> beam.io.ReadFromText(input_path)  # GCSから読み込み
              | 'ParseJson' >> beam.Map(parse_json).with_output_types(CommonLog))  # JSONデータを解析

    # この部分では、Google Cloud Storage (GCS) からデータを読み込み、JSON 形式のデータを解析しています。
    # "beam.io.ReadFromText" でテキストファイルを読み込み、 "beam.Map" で "parse_json" 関数を適用して JSON データを解析しています。
    # "p | 'ReadFromGCS' >>..." という記法は、パイプラインに処理を追加する際に使われます。


    # 4.8. 生データの書き込み
    (logs | 'RawToDict' >> beam.Map(lambda row: row._asdict())  # 辞書に変換
          | 'WriteRawToBQ' >> beam.io.WriteToBigQuery(  # BigQueryへ書き込み
              raw_table_name,
    ...
    # この部分では、生データを BigQuery に書き込んでいます。
    # "beam.Map" で "CommonLog" オブジェクトを辞書に変換し、"beam.io.WriteToBigQuery" で BigQuery に書き込んでいます。


    # 4.9. 集計結果の書き込み
    (logs | 'PerUserAggregations' >> SqlTransform(query, dialect='zetasql')  # SQL で集計
          | 'AggToDict' >> beam.Map(lambda row: row._asdict())  # 辞書に変換
          | 'WriteAggToBQ' >> beam.io.WriteToBigQuery(  # BigQueryへ書き込み
              agg_table_name,
    ...
    # この部分では、SQL を使って集計した結果を BigQuery に書き込んでいます。
    # "SqlTransform" で SQL クエリを実行し、"beam.Map" で結果を辞書に変換し、"beam.io.WriteToBigQuery" で BigQuery に書き込んでいます。
