...
def run():
    ...
    # 2.2. Beamパイプラインオプションの設定
    options = PipelineOptions()  # オプションを作成
    ...
    # 2.3. 入力と出力の設定
    input = 'gs://{0}/events.json'.format(opts.project)  # 入力パス
    output = '{0}:logs.logs'.format(opts.project)  # 出力パス

    # ここでは、入力ファイルのパスと出力先の BigQuery テーブルを指定しています。


    # 2.4. BigQueryのテーブルスキーマ
    ...
    # 2.6. データの読み込み、変換、BigQueryへの書き込み
    (p
        | 'ReadFromGCS' >> beam.io.ReadFromText(input)  # GCSから読み込み
        | 'ParseJson' >> beam.Map(lambda line: json.loads(line))  # JSONデータを解析
    ...
    # この部分では、Google Cloud Storage (GCS) からデータを読み込み、JSON 形式のデータを解析し、BigQuery に書き込んでいます。
    # "beam.io.ReadFromText" でテキストファイルを読み込み、 "beam.Map" で各行に "json.loads" 関数を適用して JSON データを解析し、"beam.io.WriteToBigQuery" で BigQuery に書き込んでいます。
